...
from ctypes import *
moveapi = cdll.LoadLibrary(os.path.expanduser('~/src/psmoveapi/build/_psmove.so'))
trackerapi = cdll.LoadLibrary(os.path.expanduser('~/src/psmoveapi/build/libpsmoveapi_tracker.so'))
...
pause_counter = 0
move_pub = rospy.Publisher("/move_info_for_test", Float32MultiArray, queue_size=100)
while True:
    reward = 0
    ...
    br.sendTransform(saved_position, saved_orientation, rospy.Time.now(), 'ball', 'world')
    move_pub.publish(move_msg)
    # print move_msg
